...
part1 :: proc(registers: ^[6]int, prgm: []Instruction, ip_reg: int) {
    ip := 0
...
part2 :: proc(registers: ^[6]int, prgm: []Instruction, ip_reg: int) -> int {
    ip := 0
    val := 0
    for ip >= 0 && ip < len(prgm) {
        if ip == 1 {
            val, _ = slice.max(registers[:])
            break
        }
        registers[ip_reg] = ip
        instr := prgm[ip]
...
main :: proc() {
    ...
    // pre parse instructions to drastically speed things up and replace parse_instruction call
    prgm := make([]Instruction, len(lines))
    parse_instructions(prgm[:], lines)

    registers: [6]int
    part1(&registers, prgm, ip_reg)

    fmt.printf("%d\n", registers[0])

    registers = 0
    registers[0] = 1
    fmt.printf("%d\n", part2(&registers, prgm, ip_reg))
}